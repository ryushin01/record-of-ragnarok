...
export default function MY_PI_002M() {
  ...
  const { mutate: savePayInfo } = useMutation({
    ...
    onSuccess: (res) => {
      if (res.code === "00") {
        ...
        // 저장성공시 currentPath -> pathCheck 로 세션에 저장
  ...
  /**
   * 형이 죽은 해부터 아버지의 정신이 이상해졌다고 했다. 유독 아끼던 자식을 잃은 슬픔. 그 감정은 이상한 형태로 변질됐다.
   * 상세한 과정은 알 수가 없었다. 그저 시간이 흘러 뉴스 속보에 나오는 유명인사가 되어버렸다는 것뿐.
   * 매달 보내주는 돈의 액수도 일정치 않았다.
   *
   * 자식들에게 복수하고 그 모습을 실황 중계하여 부모에게 보임.
   * 타겟은 자신과 같은 아들 중심이나, 예외도 존재하므로 여성 1인 포함.
   *
   * 등장인물
   * 독고만수: 사이비 교단의 교주. 첫째 아들을 잃고 흑화된 인물.
   * 독고범준: 독고만수의 둘째 아들. 모든 사건의 중심.
   * */
  ...
  function onClickNext() {
    ...
    // 상환금 금액을 0원으로 입력한 경우
    if (
      getValues().bankList.some(
        (bank) => bank.payAmt === "0" || bank.payAmt === 0
      )
    ) {
      callToast({
        msg: "0원은 입력하실 수 없습니다.",
